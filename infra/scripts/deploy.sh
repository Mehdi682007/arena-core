...
previous_release=
[[ -L "$SERVER_APP_ROOT/current" ]] && previous_release="$(readlink -f "$SERVER_APP_ROOT/current")"
activate_release() {
...
if [[ "${DEPLOY_BACKUP_ENABLED:-true}" == true ]] &&
  { [[ "$POSTGRES_MODE" == external ]] || compose --profile container-db ps --status running postgres >/dev/null 2>&1; }; then
...
if ! "$SCRIPT_DIR/verify.sh" "$inventory"; then
  ...
  if [[ -n "$previous_release" && -f "$previous_release/release/manifest.json" ]]; then
    ...
    if ARENA_VERIFY_RELEASE_VERSION="$previous_version" \
      ...
      record_deployment "$previous_version" rollback-after-failed-deploy "$failed_version"
    ...
    record_deployment "$failed_version" rollback-failed "$previous_version"
