...
if (manifest.lockfileSha256 !== (await fileHash('pnpm-lock.yaml'))) {
  fail('lockfile drift');
}
...
for (const image of images) {
  ...
  /*
   * Immutable images must contain digest pinning.
   * Example:
   * ghcr.io/org/image:tag@sha256:xxxx
   */
  if (!reference.includes('@sha256:')) {
    fail(`image ${image} is not digest pinned`);
  }

  if (reference.endsWith(':latest')) {
    fail(`image ${image} uses latest tag`);
  }
}
