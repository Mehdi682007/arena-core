...
describe('administrative RBAC', () => {
  it('has exactly 44 unique keys and covers every permission decorator', () => {
    expect(ADMIN_PERMISSION_KEYS).toHaveLength(44);
    expect(new Set(ADMIN_PERMISSION_KEYS).size).toBe(44);
  ...
  it('seeds twice without duplicates and preserves custom rows or assigning users', async () => {
    ...
    await seedSystemRbac(state.client);
    expect(state.permissions.size).toBe(45);
    expect(state.roles.get('custom')).toBeDefined();
    expect(state.rolePermissions.size).toBe(44);
